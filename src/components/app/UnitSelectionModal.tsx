...
const UnitSelectionModal = ({ isOpen, onClose, maxUnidades = 10, playerId, matchId, connectedPlayers }) => {
  ...
  const [insertPieces, { loading: insertingPieces }] = useMutation(INSERT_PIECES);
  ...
  const handleAddUnit = (unitType) => {
    if (totalUnits < maxUnidades) {
      ...
      alert(`¡Imposible desplegar más! Límite de ${maxUnidades} unidades alcanzado.`);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
      <div className="bg-gray-900 border border-amber-900/30 rounded-lg p-6 max-w-md w-full" style={{backgroundColor: '#1a1f2e'}}>
        <h3 className="text-xl font-bold text-amber-500 text-center mb-6">Seleccionar Tipo de Unidad</h3>
        
        {/* Barra de límite de unidades */}
        <div className="mb-6 px-2">
          <div className="flex justify-between mb-1">
            <span className="text-amber-500 text-sm">Capacidad de Despliegue:</span>
            <span className="text-amber-500 text-sm">{totalUnits}/{maxUnidades}</span>
          </div>
        ...
        <div className="flex justify-center space-x-8 mb-8">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-amber-500 font-bold">{unitCounts.ranged}</span>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-amber-500 font-bold">{unitCounts.normal}</span>
          </div>
        </div>
        
        <div className="text-center mb-4">
          <p className="text-amber-500 font-bold">Unidades seleccionadas: {totalUnits}</p>
        </div>
